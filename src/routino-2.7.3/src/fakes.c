...
index_t CreateFakes(Nodes * nodes, Segments * segments, int point,
		    Segment * segmentp, index_t node1, index_t node2,
		    distance_t dist1, distance_t dist2)
{
	index_t fakenode;
	...
	if (dist1 <= km_to_distance(MINSEGMENT)
	    && dist2 > km_to_distance(MINSEGMENT)) {
		prevpoint = point;
		return (node1);
	}

	if (dist2 <= km_to_distance(MINSEGMENT)
	    && dist1 > km_to_distance(MINSEGMENT)) {
		prevpoint = point;
		return (node2);
	}
	...
	if (fake_lat[point] > M_PI)
		fake_lat[point] -= 2 * M_PI;
	...
	fake_segments[4 * point - 4].distance =
	    DISTANCE(dist1) | DISTFLAG(segmentp->distance);
	...
	fake_segments[4 * point - 3].distance =
	    DISTANCE(dist2) | DISTFLAG(segmentp->distance);
	...
	if (prevpoint > 0
	    && fake_segments[4 * prevpoint - 4].node1 == node1
	    && fake_segments[4 * prevpoint - 3].node2 == node2) {
		if (DISTANCE(dist1) > DISTANCE(fake_segments[4 * prevpoint - 4].distance)) {	/* point is further from node1 than prevpoint */
			...
			fake_segments[4 * point - 2].distance =
			    (DISTANCE(dist1) -
			     DISTANCE(fake_segments[4 * prevpoint - 4].
				      distance)) | DISTFLAG(segmentp->
							    distance);
		} else {
			fake_segments[4 * point - 2] =
			    fake_segments[4 * prevpoint - 4];
			...
			fake_segments[4 * point - 2].distance =
			    (DISTANCE
			     (fake_segments[4 * prevpoint - 4].distance) -
			     DISTANCE(dist1)) | DISTFLAG(segmentp->
							 distance);
		}

		real_segments[4 * point - 2] =
		    IndexSegment(segments, segmentp);

		fake_segments[4 * prevpoint - 1] =
		    fake_segments[4 * point - 2];

		real_segments[4 * prevpoint - 1] =
		    real_segments[4 * point - 2];
	}
...
index_t CreateFakeNullSegment(Segments * segments, index_t node,
			      index_t segment, int point)
{
...
Segment *NextFakeSegment(Segment * fakesegmentp, index_t fakenode)
{
	...
	if (fakesegmentp == &fake_segments[4 * whichnode - 3]
	    && fake_segments[4 * whichnode - 2].node1 != NO_NODE)
		return (&fake_segments[4 * whichnode - 2]);

	if (fakesegmentp == &fake_segments[4 * whichnode - 3]
	    && fake_segments[4 * whichnode - 1].node1 != NO_NODE)
		return (&fake_segments[4 * whichnode - 1]);

	if (fakesegmentp == &fake_segments[4 * whichnode - 2]
	    && fake_segments[4 * whichnode - 1].node1 != NO_NODE)
		return (&fake_segments[4 * whichnode - 1]);
...
Segment *ExtraFakeSegment(index_t realnode, index_t fakenode)
{
	...
	if (fake_segments[4 * whichnode - 4].node1 == realnode
	    || fake_segments[4 * whichnode - 4].node2 == realnode)
		return (&fake_segments[4 * whichnode - 4]);

	if (fake_segments[4 * whichnode - 3].node1 == realnode
	    || fake_segments[4 * whichnode - 3].node2 == realnode)
		return (&fake_segments[4 * whichnode - 3]);
...
int IsFakeUTurn(index_t fakesegment1, index_t fakesegment2)
{
	...
	if (fake_segments[whichsegment1].node1 ==
	    fake_segments[whichsegment2].node1)
		return (1);

	if (fake_segments[whichsegment1].node2 ==
	    fake_segments[whichsegment2].node2)
		return (1);
