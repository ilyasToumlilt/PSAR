...
static index_t FindSuperSegment(Nodes * nodes, Segments * segments,
				Ways * ways, Relations * relations,
				Profile * profile, index_t finish_node,
				index_t finish_segment);
static Results *FindSuperRoute(Nodes * nodes, Segments * segments,
			       Ways * ways, Relations * relations,
			 ...
			 index_t finish_node)
{
...
#if DEBUG
	printf("  FindNormalRoute(...,start_node=%" Pindex_t
	       " prev_segment=%" Pindex_t " finish_node=%" Pindex_t ")\n",
	...
	if (IsFakeNode(finish_node))
		...
		GetLatLong(nodes, finish_node, NULL, &finish_lat,
			   &finish_lon);
	...
	result1 =
	    InsertResult(results, results->start_node,
			 results->prev_segment);
	...
	while ((result1 = PopFromQueue(queue))) {
		...
		if (profile->turns && node1p
		    && IsTurnRestrictedNode(node1p))
			turnrelation =
			    FindFirstTurnRelation2(relations, node1,
						   seg1r);
		...
		while (segmentp) {
			...
			score_t segment_pref, segment_score,
			    cumulative_score;
			int i;
			...
			if (turnrelation != NO_RELATION
			    && !IsTurnAllowed(relations, turnrelation,
					      node1, seg1r, seg2r,
					      profile->allow))
				goto endloop;
			...
			if ((wayp->height
			     && wayp->height < profile->height)
			    || (wayp->width
				&& wayp->width < profile->width)
			    || (wayp->length
				&& wayp->length < profile->length))
				goto endloop;

			segment_pref =
			    profile->highway[HIGHWAY(wayp->type)];
			...
			if (node2p && node2 != finish_node
			    && !(node2p->allow & profile->allow))
				goto endloop;

			if (option_quickest == 0)
				segment_score =
				    (score_t) DISTANCE(segmentp->
						       distance) /
				    segment_pref;
			else
				segment_score =
				    (score_t) Duration(segmentp, wayp,
						       profile) /
				    segment_pref;
			...
			if (!result2) {	/* New end node/segment combination */
				result2 =
				    InsertResult(results, node2, seg2);
...
#if DEBUG
	Result *r =
	    FindResult(results, results->start_node,
		       results->prev_segment);
...
Results *FindMiddleRoute(Nodes * nodes, Segments * segments, Ways * ways,
			 Relations * relations, Profile * profile,
			 Results * begin, Results * end)
{
...
#if DEBUG
	printf
	    ("  FindMiddleRoute(...,[begin has %d nodes],[end has %d nodes])\n",
	...
	if (!option_quiet)
		printf_first
		    ("Finding Middle Route: Super-Nodes checked = 0");
#endif
	...
	if (IsFakeNode(end->finish_node))
		...
		GetLatLong(nodes, end->finish_node, NULL, &finish_lat,
			   &finish_lon);
	...
	if (begin->number == 1) {
		if (begin->prev_segment == NO_SEGMENT)
			...
			index_t superseg =
			    FindSuperSegment(nodes, segments, ways,
					     relations, profile,
					     begin->start_node,
					     begin->prev_segment);
	...
	while (result3) {
		if ((results->start_node != result3->node
		     || results->prev_segment != result3->segment)
		    && !IsFakeNode(result3->node)
		    && IsSuperNode(LookupNode(nodes, result3->node, 3))) {
			...
			if (superseg != result3->segment) {
				result5 =
				    InsertResult(results, result3->node,
						 result3->segment);
	...
	while ((result1 = PopFromQueue(queue))) {
		...
		if (profile->turns && IsTurnRestrictedNode(node1p))	/* node1 cannot be a fake node (must be a super-node) */
			turnrelation =
			    FindFirstTurnRelation2(relations, node1, seg1);
		...
		while (segmentp) {
			...
			if ((result3 = FindResult(end, node2, seg2))) {
				if ((result2->score + result3->score) <
				    finish_score) {
					finish_score =
					    result2->score +
					    result3->score;
	...
	if (!finish_result) {
		...
		if (!option_quiet)
			printf_last
			    ("Found Middle Route: Super-Nodes checked = %d - Fail",
			     results->number);
#endif
	...
	if (finish_result->node != end->finish_node) {
		result3 =
		    InsertResult(results, end->finish_node, NO_SEGMENT);
...
#if !DEBUG
	if (!option_quiet)
		printf_last("Found Middle Route: Super-Nodes checked = %d",
			    results->number);
#endif
				...
				index_t finish_segment)
{
	...
	while (supersegmentp) {
		if (IsSuperSegment(supersegmentp)) {
			...
			results =
			    FindSuperRoute(nodes, segments, ways,
					   relations, profile, start_node,
					   finish_node);
			...
			result =
			    FindResult(results, finish_node,
				       finish_segment);

			if (result
			    && (distance_t) result->score ==
			    DISTANCE(supersegmentp->distance)) {
				FreeResultsList(results);
				return (IndexSegment
					(segments, supersegmentp));
			}
			       ...
			       index_t finish_node)
{
...
#if DEBUG
	printf("    FindSuperRoute(...,start_node=%" Pindex_t
	       " finish_node=%" Pindex_t ")\n", start_node, finish_node);
#endif
	...
	while ((result1 = PopFromQueue(queue))) {
		...
		while (segmentp) {
			...
			cumulative_score =
			    result1->score +
			    (score_t) DISTANCE(segmentp->distance);
			 ...
			 index_t finish_node)
{
...
#if DEBUG
	printf("  FindStartRoutes(...,start_node=%" Pindex_t
	       " prev_segment=%" Pindex_t " finish_node=%" Pindex_t ")\n",
	       start_node, prev_segment, finish_node);
#endif
	...
	if (results->number == 1 || nsuper == 0) {
		...
		if (!option_quiet)
			printf_last
			    ("Found Start Route: Nodes checked = %d - Fail",
			     results->number);
#endif
	...
	while (s) {
		if (s->node == finish_node) {
			Result *r =
			    FindResult(results, s->node, s->segment);

			printf("    -------- route to finish node\n");

			while (r) {
				printf("    node=%" Pindex_t " segment=%"
				       Pindex_t " score=%f\n", r->node,
				       r->segment, r->score);
...
#if !DEBUG
	if (!option_quiet)
		printf_last("Found Start Route: Nodes checked = %d",
			    results->number);
#endif
			   ...
			   Results * begin, index_t finish_node)
{
...
#if DEBUG
	printf("  ExtendStartRoutes(...,[begin has %d nodes],finish_node=%"
	...
	while (result3) {
		...
		if (!IsFakeNode(result3->node))
			if (IsSuperNode
			    (LookupNode(nodes, result3->node, 3)))
	...
	while ((result1 = PopFromQueue(queue))) {
		...
		while (segmentp) {
			...
			/* must not perform U-turn (unless profile allows) */
			if (profile->turns
			    && (seg1 == seg2 || seg1 == seg2r
				|| seg1r == seg2 || (seg1r == seg2r
						     && IsFakeUTurn(seg1,
								    seg2))))
				goto endloop;
...
#if !DEBUG
	if (!option_quiet)
		printf_last("Extended Start Route: Nodes checked = %d",
			    results->number);
#endif
			  ...
			  index_t finish_node)
{
	...
	while ((result1 = PopFromQueue(queue))) {
		...
		while (segmentp) {
			...
			if (turnrelation != NO_RELATION
				...
				if (turnrelation2 != NO_RELATION
				    && !IsTurnAllowed(relations,
						      turnrelation2, node1,
						      seg2r, seg1r,
						      profile->allow))
					goto endloop;
	...
	if (results->number == 1) {
		...
		if (!option_quiet)
			printf_last
			    ("Found Finish Route: Nodes checked = %d - Fail",
			     results->number);
#endif
	...
	while (result3) {
		if (result3->next) {
			result2 =
			    InsertResult(results2, result3->next->node,
					 result3->segment);
	...
	while (result3) {
		if (result3->next && result3->next->next) {
			result1 =
			    FindResult(results2, result3->next->node,
				       result3->segment);
			result2 =
			    FindResult(results2, result3->next->next->node,
				       result3->next->segment);
...
#if !DEBUG
	if (!option_quiet)
		printf_last("Found Finish Route: Nodes checked = %d",
			    results->number);
#endif
		       ...
		       Results * begin, Results * middle)
{
...
#if DEBUG
	printf
	    ("  CombineRoutes(...,[begin has %d nodes],[middle has %d nodes])\n",
	...
	midres =
	    FindResult(middle, middle->start_node, middle->prev_segment);

	comres1 =
	    InsertResult(combined, combined->start_node,
			 combined->prev_segment);
	...
	if (begin->number > 1 && midres->next) {
		...
		midres =
		    FindResult(middle, midres->next->node,
			       midres->next->segment);
		...
		begres =
		    FindResult(begin, begin->start_node,
			       begin->prev_segment);
		...
		do {
			...
			comres2 =
			    InsertResult(combined, begres->node,
					 begres->segment);
	...
	do {
		...
		if (midres->next) {
			Results *results =
			    FindNormalRoute(nodes, segments, ways,
					    relations, profile,
					    comres1->node,
					    comres1->segment,
					    midres->next->node);
			...
			if (!results) {
				...
				if (!option_quiet)
					printf_last
					    ("Found Combined Route: Nodes = %d - Fail",
					     combined->number);
#endif
			...
			result =
			    FindResult(results, midres->node,
				       comres1->segment);
			...
			do {
				...
				comres2 =
				    InsertResult(combined, result->node,
						 result->segment);

				comres2->score =
				    midres->score + result->score;
				comres2->prev = comres1;
...
#if DEBUG
	Result *r =
	    FindResult(combined, combined->start_node,
		       combined->prev_segment);
...
#if !DEBUG
	if (!option_quiet)
		printf_last("Found Combined Route: Nodes = %d",
			    combined->number);
#endif
