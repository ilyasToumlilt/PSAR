...
static TaggingRuleList *AppendTaggingRule(TaggingRuleList * rules,
					  const char *k, const char *v,
					  int action);
static void AppendTaggingAction(TaggingRuleList * rules, const char *k,
				const char *v, int action,
				const char *message);
static void DeleteTaggingRuleList(TaggingRuleList * rules);

static void ApplyRules(TaggingRuleList * rules, TagList * input,
		       TagList * output, const char *match_k,
		       const char *match_v);
...
static int IfType_function(const char *_tag_, int _type_, const char *k,
			   const char *v);
static int IfNotType_function(const char *_tag_, int _type_, const char *k,
			      const char *v);
static int SetType_function(const char *_tag_, int _type_, const char *k,
			    const char *v);
static int UnsetType_function(const char *_tag_, int _type_,
			      const char *k);
static int OutputType_function(const char *_tag_, int _type_,
			       const char *k, const char *v);
static int LogErrorType_function(const char *_tag_, int _type_,
				 const char *k, const char *v,
				 const char *message);
...
static xmltag *xml_toplevel_tags[] =
    { &xmlDeclaration_tag, &RoutinoTaggingType_tag, NULL };
...
static xmltag RoutinoTaggingType_tag = { "routino-tagging",
	0, {NULL},
	NULL,
...
static xmltag NodeType_tag = { "node",
	0, {NULL},
	NodeType_function,
...
static xmltag RelationType_tag = { "relation",
	0, {NULL},
...
/*+ The IfType type tag. +*/
static xmltag IfType_tag = { "if",
	2, {"k", "v"},
	IfType_function,
	{&IfType_tag, &IfNotType_tag, &SetType_tag, &UnsetType_tag,
	 &OutputType_tag, &LogErrorType_tag, NULL}
};

/*+ The IfNotType type tag. +*/
static xmltag IfNotType_tag = { "ifnot",
	2, {"k", "v"},
	IfNotType_function,
	{&IfType_tag, &IfNotType_tag, &SetType_tag, &UnsetType_tag,
	 &OutputType_tag, &LogErrorType_tag, NULL}
...
static xmltag UnsetType_tag = { "unset",
	1, {"k"},
	UnsetType_function,
	{NULL}
};
...
static xmltag OutputType_tag = { "output",
	2, {"k", "v"},
	OutputType_function,
	{NULL}
};
...
static xmltag LogErrorType_tag = { "logerror",
	3, {"k", "v", "message"},
			   ...
			   const char *v)
{
	if (_type_ & XMLPARSE_TAG_START) {
		if (!current_list_stack
		    || (current_list_stack_depth % 8) == 7)
			current_list_stack =
			    (TaggingRuleList **) realloc((void *)
							 current_list_stack,
							 (current_list_stack_depth
							  +
							  8) *
							 sizeof
							 (TaggingRuleList
		...
		current_list_stack[current_list_stack_depth++] =
		    current_list;

		current_list =
		    AppendTaggingRule(current_list, k, v, TAGACTION_IF);
	}
			      ...
			      const char *v)
{
	if (_type_ & XMLPARSE_TAG_START) {
		...
		current_list =
		    AppendTaggingRule(current_list, k, v, TAGACTION_IFNOT);
	}
...
int ParseXMLTaggingRules(const char *filename)
{
	...
	if (!ExistsFile(filename)) {
		fprintf(stderr,
			"Error: Specified tagging rules file '%s' does not exist.\n",
			filename);
		return (1);
	}
...
TaggingRuleList *AppendTaggingRule(TaggingRuleList * rules, const char *k,
				   const char *v, int action)
{
	if ((rules->nrules % 16) == 0)
		rules->rules =
		    (TaggingRule *) realloc((void *) rules->rules,
					    (rules->nrules +
					     16) * sizeof(TaggingRule));
	...
	if (k)
		rules->rules[rules->nrules - 1].k =
		    strcpy(malloc(strlen(k) + 1), k);
	else
		rules->rules[rules->nrules - 1].k = NULL;

	if (v)
		rules->rules[rules->nrules - 1].v =
		    strcpy(malloc(strlen(v) + 1), v);
	else
		rules->rules[rules->nrules - 1].v = NULL;

	rules->rules[rules->nrules - 1].message = NULL;

	rules->rules[rules->nrules - 1].rulelist =
	    (TaggingRuleList *) calloc(sizeof(TaggingRuleList), 1);
				...
				const char *message)
{
	...
	if (message)
		rules->rules[rules->nrules - 1].message =
		    strcpy(malloc(strlen(message) + 1), message);
	else
		rules->rules[rules->nrules - 1].message =
		    default_logerror_message;
...
void DeleteTaggingRuleList(TaggingRuleList * rules)
{
	...
	for (i = 0; i < rules->nrules; i++) {
		...
		if (rules->rules[i].message
		    && rules->rules[i].message != default_logerror_message)
...
void AppendTag(TagList * tags, const char *k, const char *v)
{
	if ((tags->ntags % 8) == 0) {
		...
		tags->k =
		    (char **) realloc((void *) tags->k,
				      (tags->ntags + 8) * sizeof(char *));
		tags->v =
		    (char **) realloc((void *) tags->v,
				      (tags->ntags + 8) * sizeof(char *));
	...
	tags->k[tags->ntags] =
	    strcpy(realloc(tags->k[tags->ntags], strlen(k) + 1), k);
	tags->v[tags->ntags] =
	    strcpy(realloc(tags->v[tags->ntags], strlen(v) + 1), v);
...
void ModifyTag(TagList * tags, const char *k, const char *v)
{
	...
	for (i = 0; i < tags->ntags; i++)
		if (!strcmp(tags->k[i], k)) {
			tags->v[i] =
			    strcpy(realloc(tags->v[i], strlen(v) + 1), v);
			return;
		}
...
char *StringifyTag(TagList * tags)
{
	...
	for (i = 0; i < tags->ntags; i++)
		used +=
		    sprintf(string + used, "<tag k='%s' v='%s'>",
			    tags->k[i], tags->v[i]);
		       ...
		       const char *match_v)
{
	...
	if (match_k)
		match_k_copy =
		    strcpy(malloc(strlen(match_k) + 1), match_k);

	if (match_v)
		match_v_copy =
		    strcpy(malloc(strlen(match_v) + 1), match_v);
	...
	for (i = 0; i < rules->nrules; i++) {
		...
		case TAGACTION_LOGERROR:
			...
			if (current_list == &NodeRules)
				logerror("Node %" Pnode_t
					 " has an unrecognised tag '%s' = '%s' (in tagging rules); %s.\n",
					 logerror_node(current_id), k, v,
					 rules->rules[i].message);
			if (current_list == &WayRules)
				logerror("Way %" Pway_t
					 " has an unrecognised tag '%s' = '%s' (in tagging rules); %s.\n",
					 logerror_way(current_id), k, v,
					 rules->rules[i].message);
			if (current_list == &RelationRules)
				logerror("Relation %" Prelation_t
					 " has an unrecognised tag '%s' = '%s' (in tagging rules); %s.\n",
					 logerror_relation(current_id), k,
					 v, rules->rules[i].message);
		}
	}
