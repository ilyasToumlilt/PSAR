...
Results *NewResultsList(uint8_t log2bins)
{
	...
	results->count =
	    (uint8_t *) calloc(results->nbins, sizeof(uint8_t));
	results->point =
	    (Result **) calloc(results->nbins, sizeof(Result *));
...
Result *InsertResult(Results * results, index_t node, index_t segment)
{
	...
	if (results->count[bin] == results->ncollisions) {
		...
		results->count =
		    (uint8_t *) realloc((void *) results->count,
					results->nbins * sizeof(uint8_t));
		results->point =
		    (Result **) realloc((void *) results->point,
					results->nbins * sizeof(Result *));

		log_malloc(results->count,
			   results->nbins * sizeof(uint8_t));
		log_malloc(results->point,
			   results->nbins * sizeof(Result *));
	...
	if ((results->number % results->ndata2) == 0) {
		...
		if (results->ndata1 >= results->nallocdata1) {
			results->nallocdata1++;
			results->data =
			    (Result **) realloc((void *) results->data,
						results->nallocdata1 *
						sizeof(Result *));
			results->data[results->nallocdata1 - 1] =
			    (Result *) malloc(results->ndata2 *
					      sizeof(Result));
			log_malloc(results->data[results->nallocdata1 - 1],
				   results->ndata2 * sizeof(Result));
		}
	}
	...
	result =
	    &results->data[results->ndata1 -
			   1][results->number % results->ndata2];
